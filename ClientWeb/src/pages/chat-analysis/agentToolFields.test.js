...
//   2. agent_tool_info 列渲染纯函数（长截断 + 空降级 + title 属性）
...
// ===== 2. agent_tool_info 列渲染纯函数（与 index.jsx 的 render 行为一致） =====
// 重构自 index.jsx 第 144-148 行的 render 表达式：
//   v => v ? <span title={v}>{String(v).length > 60 ? String(v).slice(0, 60) + '…' : v}</span> : '-'
function renderAgentToolInfo(v) {
  if (!v) return '-'
  const s = String(v)
  if (s.length > 60) {
...
ok(renderAgentToolInfo(null) === '-', 'null → "-"')
ok(renderAgentToolInfo(undefined) === '-', 'undefined → "-"')
ok(renderAgentToolInfo('') === '-', '空字符串 → "-"')

// 2.2 短字符串（含版本号 / 运行时）原样展示 + title 属性完整
const short = 'opencode v1.2.3'
const shortOut = renderAgentToolInfo(short)
...
const longOut = renderAgentToolInfo(long)
...
function buildAgentBlock(row) {
  ...
  if (row.agent_tool_info) items.push({ key: 'info', label: 'agentToolInfo', value: row.agent_tool_info })
  if (row.agent_tool_session_id) items.push({ key: 'session', label: 'agentSessionId', value: row.agent_tool_session_id })
  return items.length ? items : null
}

// 3.1 三字段全空 → 不渲染
ok(buildAgentBlock({ agent_tool_name: '', agent_tool_info: '', agent_tool_session_id: '' }) === null, '三字段全空 → 不渲染')
...
const onlyInfo = buildAgentBlock({ agent_tool_info: 'opencode v1.2.3 / node v20' })
ok(Array.isArray(onlyInfo) && onlyInfo.length === 1, '仅 agent_tool_info → 1 项')
ok(onlyInfo[0].key === 'info' && onlyInfo[0].value === 'opencode v1.2.3 / node v20', 'info 项 value 匹配')
...
const all = buildAgentBlock({
  ...
  agent_tool_info: 'Claude Code 1.0.27 / darwin arm64',
...
// 3.4 任意两字段组合
const nameInfo = buildAgentBlock({ agent_tool_name: 'opencode', agent_tool_info: 'v0.5.0' })
...
const infoSess = buildAgentBlock({ agent_tool_info: 'v1.0', agent_tool_session_id: 'sess-000' })
