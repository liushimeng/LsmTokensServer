...
//   2. Agent工具定义 列渲染纯函数（数据源 request_tools 字段；长截断 + 空降级 + title 属性）
...
// v2.0.7x：该列数据源由 agent_tool_info 字段切换为 request_tools 字段，渲染逻辑不变。
// 重构自 index.jsx 的 render 表达式：
//   v => v ? <span title={v}>{String(v).length > 60 ? String(v).slice(0, 60) + '…' : v}</span> : '-'
function renderRequestTools(v) {
  if (!v) return '-'
  const s = String(v)
  if (s.length > 60) {
...
ok(renderRequestTools(null) === '-', 'null → "-"')
ok(renderRequestTools(undefined) === '-', 'undefined → "-"')
ok(renderRequestTools('') === '-', '空字符串 → "-"')

// 2.2 短字符串（含版本号 / 运行时）原样展示 + title 属性完整
const short = 'web_search,code_interpreter,file_read'
const shortOut = renderRequestTools(short)
...
const longOut = renderRequestTools(long)
...
// v2.0.7x：详情块「Agent工具定义」项数据源由 agent_tool_info 字段切换为 request_tools 字段。
// 重构自 DetailHeader.jsx 的三元判断：
//   const hasAny = row.agent_tool_name || row.request_tools || row.agent_tool_session_id
//   const itemName = row.agent_tool_name ? { label: '...', value: row.agent_tool_name } : null
//   const itemInfo = row.request_tools ? { ... } : null
...
function buildAgentBlock(row) {
  ...
  if (row.request_tools) items.push({ key: 'info', label: 'agentToolInfo', value: row.request_tools })
  if (row.agent_tool_session_id) items.push({ key: 'session', label: 'agentSessionId', value: row.agent_tool_session_id })
  return items.length ? items : null
}

// 3.1 三字段全空 → 不渲染
ok(buildAgentBlock({ agent_tool_name: '', request_tools: '', agent_tool_session_id: '' }) === null, '三字段全空 → 不渲染')
...
const onlyInfo = buildAgentBlock({ request_tools: 'web_search,code_interpreter,file_read' })
ok(Array.isArray(onlyInfo) && onlyInfo.length === 1, '仅 request_tools → 1 项')
ok(onlyInfo[0].key === 'info' && onlyInfo[0].value === 'web_search,code_interpreter,file_read', 'info 项 value 匹配')
...
const all = buildAgentBlock({
  ...
  request_tools: 'web_search,code_interpreter,file_read',
...
// 3.4 任意两字段组合
const nameInfo = buildAgentBlock({ agent_tool_name: 'opencode', request_tools: 'search,read' })
...
const infoSess = buildAgentBlock({ request_tools: 'shell,browser', agent_tool_session_id: 'sess-000' })
