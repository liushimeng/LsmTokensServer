// 登录态与本地偏好存储
// 阶段BP（20260903）：v6 存储结构——用户名登录条目 ph（手机号）字段加密存储，
// 使用 Web Crypto API AES-GCM 加密，避免明文手机号暴露在 localStorage；
// 兼容 v5（明文 ph）/v4（无 ph）与 v3 迁移。
...
// 阶段BP：显式 .js 后缀，兼容 node 直跑自检脚本（vite 构建同样支持）
import { baseUrl } from './api.js'
import { encrypt, decrypt, isEncrypted } from './crypto.js'

const STORAGE_KEY = 'lsm_agent_creds'

// saveCredentials 保存登录凭据到 localStorage（async：手机号需加密）
// @param {string} loginType - 登录类型：'model' 或 'user'
// @param {string} name - 模型名或用户名
// @param {string} [phone] - 手机号（仅 user 登录有效；加密存储）
export async function saveCredentials(loginType, name, phone) {
  try {
    const stored = localStorage.getItem(STORAGE_KEY)
    let data = stored ? JSON.parse(stored) : null
    // 阶段AS：同时保留两个 Tab 的凭据，各存各的，互不覆盖。
    if (!data || !data.v4) {
      // 迁移旧数据或初始化：旧 v3 格式只存一份 → 迁移到 v6 双份
    ...
    // 去掉旧 v3 字段，避免歧义
    delete data.v3
    // 阶段BP：版本号升 v6（user 条目 ph 字段加密存储），读取侧兼容 v5/v4
    data.v = 6
    ...
    if (loginType === 'model') {
      ...
      // 阶段BP：手机号加密存储（若为空则保留原有密文，解密时返回空）
      let encryptedPh = ''
      const phoneToStore = phone || (isEncrypted(prevPh) ? prevPh : '')
      if (phoneToStore) {
        encryptedPh = isEncrypted(phoneToStore) ? phoneToStore : await encrypt(phoneToStore)
      }
      data.user = { mn: name || '', ph: encryptedPh, ts: Date.now() }
      data.active = 'user'
    }
...
// loadCredentials 从 localStorage 加载登录凭据（async：手机号需解密）
// @returns {Promise<null | { loginType: string, modelName: string, modelSaved: boolean, userSaved: boolean }>}
export async function loadCredentials() {
  try {
    ...
    // 阶段BP v6 / 阶段BO v5 / 阶段AS v4：双份存储，返回最近 active 的 Tab 对应的凭据
    // （v6 的 user 条目 ph 字段为加密密文，需解密后返回）
    if ((data.v === 4 || data.v === 5 || data.v === 6) && data.active) {
      const activeData = data.active === 'user' ? data.user : data.model
      const mn = activeData && activeData.mn ? activeData.mn : ''

      // 阶段BP：解密手机号（v6 密文 / v5 明文 自动识别）
      let decryptedUserPh = ''
      if (data.user && data.user.ph) {
        try {
          decryptedUserPh = isEncrypted(data.user.ph)
            ? await decrypt(data.user.ph)
            : data.user.ph // v5 明文兼容
        } catch {
          // 解密失败时视为空（防止损坏数据导致整体读取失败）
          decryptedUserPh = ''
        }
      }
      ...
      return {
        ...
        userData:  data.user  ? { ...data.user, ph: decryptedUserPh } : null,
      }
    }

    // 旧版记录（v:1 含加密 ak / v:2 无 loginType / v:3 单份）→ 尝试迁移为 v6
    if (data.v === 3) {
